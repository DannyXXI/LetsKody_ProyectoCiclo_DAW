...
/*
        ...
        $resultado = []; // inicializamos el array que contendra el resultado de la busqueda
        ...
        foreach ($puntuacionesBanderas as $puntuacion) {
            // si el id del usuario coincide con el campo 'usuario_id' se registra sus datos y posicion en el array
            if ($puntuacion->usuario_id == $id) {
                $resultado['puntuaciones'].array_push([
                    'aciertos' => $puntuacion->aciertos,
                    'fallos' => $puntuacion->fallos,
                    'preguntas_saltadas' => $puntuacion->preguntas_saltadas,
                    'puntos' => $puntuacion->puntos,
                    'tiempo' => $puntuacion->tiempo,
                    'posicion' => $posicionBanderas,
        ...
        // antes de mandar el array como JSON al frontend comprobamos si esta vacio
        if(empty($resultado)) {
            return response()->json([
                "tienePuntuacion" => false,
                "mensaje" => "⚠️​​ El usuario con ID " . $id . " no tiene ninguna puntuación registrada."
            ],200);
        }
        else {
            return response()->json($resultado, 200); // se devuelve al frontend el array convertido en JSON
        }
            */
